...
```{r fiber strength}
ggplot(data =field, aes(x= strength_gtex))+
  geom_density()
...
daymet_one <- download_daymet(site = field$site[[1]], lat = field$lat[[1]], lon = field$lon[[1]], start = field$year[[1]], end = field$year[[1]], simplify = T) 
...
datmet_extracred <- extracred %>%
  pmap(list(.y=year, .s=site, .la= lat, .lon = lon))
