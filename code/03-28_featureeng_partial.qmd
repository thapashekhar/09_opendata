...
fe_month <- fieldweather %>%
  ...
  mutate(month_abb = lubridate::month(date, label =T))
...
fe_month_sum <- fe_month%>%
  ...
  summarise(across(.cols = c(dayl.s, srad.wm2,tmax.c,tmin.c,vp.pa), mean, .names = "mean_{.col}"),
...
library(ggridges)
ggplot(data = fe_month_sum, aes(x = mean_tmax.c, y = month_abb, fill = stat(x)))+
...
finalplots <- fe_month_sum %>%
  ...
  mutate(plot = map2(data, name, ~
                       ggplot(data = .x, aes(x = value, y = month_abb, fill = stat(x)))+
                          geom_density_ridges_gradient(scale =3, rel_min_height = 0.01)+
                          scale_fill_viridis_c(option = "C")+
                          theme(legend.position = "none")+
                       labs(x=.y)))
